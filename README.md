...
This repository is prepared for the `0.12.1` release. This addon targets TBC
Anniversary only. The `0.12.1` package uses WoW interface `20505`, matching the
...
The `0.12.1` release ships with generated data from audited local Wowhead TBC
...
- No profile import/export is included in `0.12.1`.
...
```powershell
.\scripts\check-release.ps1 -Version 0.12.1 -FullData
```

Use the release-only `CHANGELOG.md` as the GitHub release notes. CurseForge
