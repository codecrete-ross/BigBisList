...
This repository is prepared for the `0.12.0` release. This addon targets TBC
Anniversary only. The `0.12.0` package uses WoW interface `20505`, matching the
...
The `0.12.0` release ships with generated data from audited local Wowhead TBC
...
- No profile import/export is included in `0.12.0`.
...
```powershell
.\scripts\check-release.ps1 -Version 0.12.0 -FullData
```

Use only the matching current changelog section as the GitHub release notes.
CurseForge release files are generated by the repository webhook from the
pushed Git tag. Do not commit generated zip files or use a GitHub release asset
as the CurseForge source.
