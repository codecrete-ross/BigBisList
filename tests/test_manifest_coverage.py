...
class ManifestCoverageTests(unittest.TestCase):
    def test_expected_manifest_units_cover_every_static_data_family(self):
        ...
        self.assertEqual(len(units), 843)
        ...
        self.assertEqual(sum(1 for unit in units if unit["data_family"] == "item_corpus"), 1)
    ...
    def test_current_manifest_covers_remaining_scrape_scope_without_fetching(self):
        ...
        self.assertEqual(coverage["expected_units"], 843)
        self.assertEqual(coverage["present_units"], 843)
        ...
        self.assertEqual(coverage["by_family"]["item_corpus"]["present"], 1)
